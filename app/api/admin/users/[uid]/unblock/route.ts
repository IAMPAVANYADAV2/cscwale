...
export async function POST(
  ...
  try {
    ...
    const user = await usersService.unblock(uid, auth.uid);

    const { response, status } = successResponse(user, "User unblocked successfully");
    return NextResponse.json(response, { status });
  } catch (error) {
    console.error("User unblock error:", error);
    const err = error instanceof Error ? error.message : "Failed to unblock user";
